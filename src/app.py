...
from module.emby import get_item_images, set_item_image # 项目图片 
...
def update_media_info(mediaId, forceUpdate = False):
    mediaDetail = get_media_details(mediaId)
    if not forceUpdate or mediaDetail['LockData']:
        # 项目已被锁,且不强制更新的情况 
        return 1
    imageList = get_item_images(mediaId)
    if update_media_with_douban(mediaDetail, imageList, set_item_image):
...
def update_role_info(roleId, forceUpdate = False):
    ...
    if not forceUpdate or fieldRole['LockData']:
        # 项目已被锁,且不强制更新的情况 
        return 1
    imageList = get_item_images(mediaId)
    if update_role_with_douban(fieldRole, imageList, set_item_image):
